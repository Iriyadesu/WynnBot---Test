...
class Moderation(commands.Cog):
    ...
    async def ban(self, ctx, user: discord.Member, *, reason="No reason provided"):
        ...
        channel = self.bot.get_channel(782625707963842600)
        await channel.send(f'User {user.display_name} was banned. Reason: {reason}')
    ...
    async def kick(self, ctx, user: discord.Member, *, reason="No reason provided"):
        ...
        channel = self.bot.get_channel(782625707963842600)
        await channel.send(f'User {user.mention} was kicked. Reason: {reason}')
    ...
    async def mute(self, ctx, user: discord.Member, *, reason='No reason provided'):
        ...
        channel = self.bot.get_channel(782625707963842600)
        await channel.send(f'User {user.mention} was muted. Reason: {reason}')
